...
class Specs:
    ...
    def export(self) -> Self:
        file_path = os.path.join(os.path.dirname(__file__), f"{self.__name}.py")
        with open(file_path, "w") as f:
            f.write(
                f"'''Mocking specs for {self.__raw_name} (automatically generated by {__file__})'''"
            )
            f.write("\n\n")
            f.write("METADATA = ")
...
if __name__ == "__main__":
    Specs(
        "D:\\Documents\\real_estate\\data\\Zip_ZORI_AllHomesPlusMultifamily_Smoothed.csv"
    ).extract_column_metadata().export()
